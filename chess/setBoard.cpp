...
bool setBoard::validFen(string testString, fen &makeFen){
    ...
    //Board string
    s >> word;
    if(not validBoard(word, fenBoard)){ return false; }
    tempF.updateBoard(fenBoard);

    //Turn string
    s >> word;
    if(not tempF.setTurn(word)){ return false; }

    //Castle string
    s >> word;
    if(not tempF.updateCastle(word)){ return false; }

    //Pasant string
    s >> word;
    if(not tempF.updatePessant(word)){ return false; }
    
    //50Moves string
    s >> word;
    if(not tempF.update50(word)){ return false; }
    
    //All moves string
    s >> word;
    if(not tempF.updateAllMoves(word)){ return false; }
...
//Takes first string and converts to board if able
bool setBoard::validBoard(string boardStr, piece fenBoard[][8]){
    ...
    setFullBoard(boardStr, fenBoard);
    return true;
}
...
void setBoard::clearBoard(piece useBoard[][8]){
    for (int i = 0; i < 64; i++)
    {
        useBoard[i%8][i/8].player = 2;
        useBoard[i%8][i/8].name = '.';
    }  
}
...
string setBoard::deletePiece(string line, piece useBoard[][8]){
    if((line[6] != ' ') or (line[7] < 'A') or (line[7] > 'H') or 
        ...
        useBoard[7 - (line[8] - '1')][line[7] - 'A'].name = '.';
        useBoard[7 - (line[8] - '1')][line[7] - 'A'].player = 2;
...
bool setBoard::placePiece(string line, piece useBoard[][8], string tryName, 
    ...
    if(pieceName != tryName){
        ...
        (square[1] < '1') or (square[1] > '8')){
            ...
            if(pieceName != "Knight"){
                useBoard[7 - (square[1] - '1')][square[0] - 'A'].name = pieceName[0];
            } else {
                useBoard[7 - (square[1] - '1')][square[0] - 'A'].name = 'N';
            }
            useBoard[7 - (square[1] - '1')][square[0] - 'A'].player = color;
